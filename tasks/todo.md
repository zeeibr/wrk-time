...
- [x] Add `MovePattern` (squat, hinge, lunge, pushH, pushV, pullH, pullV, carry,
...
- [x] Test: every strength move has a pattern and a position; every pattern has
      at least one move per owned-kit level.

### Phase 2 — Sessions that flow
- [x] `MoveLibrary.rotation` (the single builder — not a second one) gains a
